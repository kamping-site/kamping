...
static constexpr bool is_vector_bool_v<
    T, typename std::enable_if<!has_value_type_v<
           std::remove_cv_t<std::remove_reference_t<T>>>>::type> = false;
...
static constexpr bool is_vector_bool_v<
    ...
    is_specialization<
        std::remove_cv_t<std::remove_reference_t<T>>, std::vector>::value&&
        std::is_same_v<
...
public:
    ...
    static constexpr bool is_modifiable =
        modifiability
        == BufferModifiability::modifiable; ///< Indicates whether the
    ...
    using value_type =
        typename ValueTypeWrapper<!is_single_element, MemberType>::
    ...
    DataBuffer() : _data() {
        ...
        static_assert(
            is_modifiable, "Lib allocated buffers must be modifiable"
        );
    }
    ...
    void resize(size_t size) {
        ...
        if constexpr (is_single_element) {
            KASSERT(
                size == 1u, "Cannot resize a single element buffer to hold "
    ...
    value_type const get_single_element() const {
        kassert_not_extracted("Cannot get an element from a buffer that has "
    ...
    MemberTypeWithConst extract() {
        static_assert(
            ownership == BufferOwnership::owning,
            "Moving out of a reference should not be done because it would "
            "leave "
...
private:
    ...
    void kassert_not_extracted(std::string const message [[maybe_unused]]
    ) const {
#if KASSERT_ENABLED(KAMPING_ASSERTION_LEVEL_NORMAL)
...
public:
    static constexpr ParameterType parameter_type =
        ParameterType::root; ///< The type of parameter this object
                             ///< encapsulates.
