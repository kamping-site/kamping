...
///
/// The non-modifiable buffers encapsulate input data like data to send or send counts needed for a
/// lot of \c MPI calls. If the user already computed additional information like the send
/// displacements or receive counts for a collective operations that would otherwise have to be
/// computed by the library, these values can also be provided to the library via non-modifiable
/// buffers.
///
/// The modifiable buffers provide memory to store the result of \c MPI calls and
/// (intermediate information needed to complete an \c MPI call like send displacements or receive
/// counts/displacements etc. if the user has not yet provided them). The storage can be either
/// provided by the user or can be allocated by the library.
///
...
/// - but this may only be matched in the case that <tt>Template<Args...> = std::vector<bool,
/// my_alloc></tt>
/// @tparam T The concrete type.
/// @tparam Template the type template
/// @return \c true if the type is an instance and \c false otherwise.
...
/// @brief Boolean value helping to check if a type is an instance of \c std::vector<bool>.
/// This catches the edge case of elements which do not have a value type, they can not be a vector
/// bool.
///
/// @tparam T The type.
...
static constexpr bool is_vector_bool_v<
    ...
    typename std::enable_if<
        !has_value_type_v<std::remove_cv_t<std::remove_reference_t<T>>>>::type> = false;
...
static constexpr bool is_vector_bool_v<
    T,
    typename std::enable_if<has_value_type_v<std::remove_cv_t<std::remove_reference_t<T>>>>::type> =
...
///
/// DataBuffer wraps all buffer storages provided by an std-like container like std::vector or
/// single values. A Container type must provide \c data(), \c size() and expose the type definition
/// \c value_type.
...
public:
    ...
    static constexpr bool is_modifiable =
        modifiability == BufferModifiability::modifiable; ///< Indicates whether the underlying
                                                          ///< storage is modifiable.
    static constexpr bool is_single_element =
        !has_data_member_v<MemberType>; ///<`true` if the DataBuffer represents a singe element,
                                        ///<`false` if the
                                        ///< DataBuffer represents a container.
    using MemberTypeWithConst =
        std::conditional_t<is_modifiable, MemberType, MemberType const>; ///< The ContainerType as
                                                                         ///< const or non-const
                                                                         ///< depending on
                                                                         ///< modifiability.

    // We can not do the check for std::vector<bool> here, because to use a DataBuffer of
    // std::vector<bool> as an unused default parameter is allowed, as long the buffer is never
    // used. Therefore the check for std::vector<bool> happens only when the underlying member is
    // actually accessed and the corresponding accessor method is instantiated. static_assert(
    //     !is_vector_bool_v<MemberType>,
    //     "Buffers based on std::vector<bool> are not supported, use std::vector<kamping::kabool>
    //     instead.");
    ...
    using MemberTypeWithConstAndRef = std::conditional_t<
        ...
        MemberTypeWithConst&>; ///< The ContainerType as const or non-const (see
                               ///< ContainerTypeWithConst) and reference or non-reference depending
                               ///< on ownership.

    using value_type =
        typename ValueTypeWrapper<!is_single_element, MemberType>::value_type; ///< Value type of
                                                                               ///< the buffer.
    // Logical implication: is_int_type(type) => std::is_same_v<value_type, int>
    static_assert(
        !is_int_type(type) || std::is_same_v<value_type, int>, "The given data must be of type int"
    );
    using value_type_with_const =
        std::conditional_t<is_modifiable, value_type, value_type const>; ///< Value type as const or
    ...
    /// @brief Resizes the underlying container such that it holds exactly \c size elements of \c
    /// value_type if the \c MemberType is not a \c Span or a single elements.
    ///
    /// This function calls \c resize on the container if the container is not of type \c Span or a
    /// single value. If the container is a \c Span,  KaMPIng assumes that the memory is managed by
    /// the user and that resizing is not wanted. In this case it is \c KASSERTed that the memory
    /// provided by the span is sufficient. If the buffer stores only a single value, it is
    /// KASSERTed that the requested size is exactly 1. Whether new memory is allocated and/or data
    ...
    void resize(size_t size) {
        // This works because in template classes, only functions that are actually called are
        // instantiated Technically not needed here because _data is const in this case, so we can't
        // call resize() anyways. But this gives a nicer error message.
        ...
        if constexpr (is_single_element) {
            KASSERT(
                size == 1u,
                "Cannot resize a single element buffer to hold zero or more than one element. "
                "Single "
...
private:
    /// @brief Set the extracted flag to indicate that the data stored in this buffer has been moved
    /// out.
    void set_extracted() {
    ...
    /// @brief Throws an assertion if the extracted flag is set, i.e. the underlying data has been
    /// moved out.
    ///
...
#if KASSERT_ENABLED(KAMPING_ASSERTION_LEVEL_NORMAL)
    bool is_extracted =
        false; ///< Has the container been extracted and is therefore in an invalid state?
#endif
};
...
/// @brief Encapsulates rank of the root PE. This is needed for \c MPI collectives like \c
/// MPI_Gather.
class Root {
public:
    static constexpr ParameterType parameter_type =
...
/// This wraps an MPI operation without the argument of the operation specified. This enables the
/// user to construct such wrapper using the parameter factory \c kamping::op without passing the
/// type of the operation. The library developer may then construct the actual operation wrapper
/// with a given type later.
///
...
public:
    ...
    /// @param commutative_tag tag indicating if the operation is commutative (see \c kamping::op
    /// for details)
