...
///
/// Other than the operators defined in `<functional>` like \c std::plus, \c
/// std::max is a function and not a function object. To enable template
/// matching for detection of builtin MPI operations we therefore need to wrap
/// it. The actual implementation is used in case that the operation is a
...
/// type parameter, which leaves the operand type to be deduced.
///
/// The actual implementation is used in case that the operation is a builtin
...
///
/// Other than the operators defined in `<functional>` like \c std::plus, \c
/// std::min is a function and not a function object. To enable template
/// matching for detection of builtin MPI operations we therefore need to wrap
/// it. The actual implementation is used in case that the operation is a
/// builtin operation for the given datatype.
///
...
/// type parameter, which leaves the operand type to be deduced. The actual
/// implementation is used in case that the operation is a builtin operation for
/// the given datatype.
template <>
...
/// provided a function object for it.
///
/// The actual implementation is used in case that the operation is a builtin
/// operation for the given datatype.
///
...
/// without type parameter, which leaves to operand type to be deduced. The
/// actual implementation is used in case that the operation is a builtin
/// operation for the given datatype.
template <>
...
///
/// You can either use them by passing their STL counterparts like \c
/// std::plus<>, \c std::multiplies<> etc. or using the aliases \c
/// kamping::ops::plus, \c kamping::ops::multiplies, \c kamping::ops::max(), ...
/// You can either use them without a template parameter (\c std::plus<>) or
/// explicitly specify the type (\c std::plus<int>). In the latter case, the
/// type must match the datatype of the buffer the operation shall be applied
/// to.
namespace ops {
...
/// @brief tag for a reduce operation without manually declared commutativity
/// (this is only used internally for builtin reduce operations)
...
/// @brief Type trait for checking whether a functor is a builtin MPI reduction
/// operation and query corresponding \c MPI_Op.
///
/// Example:
/// @code
...
/// //is_builtin_mpi_op<std::minus<>, int>::op()        // error: fails to
/// compile because op is not defined
/// @endcode
///
...
/// @brief Wrapper for a user defined reduction operation based on a functor
/// object.
///
...
public:
    static_assert(
        ...
        "This wrapper only works with default constructible functors, i.e., "
        "not with lambdas."
    );
...
public:
    ...
    UserOperationPtrWrapper<is_commutative>(UserOperationPtrWrapper<
                                            is_commutative> const&) = delete;
...
/// @brief Wraps an operation and translates it to a builtin \c MPI_Op or
/// constructs a custom operation.
...
class ReduceOperation {
    static_assert(
        ...
        "For custom operations you have to specify whether they are "
        "commutative."
    );
