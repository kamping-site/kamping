...
///
inline MPI_Datatype
construct_custom_continuous_type(size_t const num_bytes_unsigned) {
...
[[nodiscard]] MPI_Datatype mpi_custom_continuous_type() KAMPING_NOEXCEPT {
  ...
  // declaration, this is thread safe.
  static MPI_Datatype type =
    internal::construct_custom_continuous_type(NumBytes);
  // From the second call onwards, re-use the existing type.
  return type;
}

/// @brief the members specify which group the datatype belongs to according to
/// the type groups specified in Section 5.9.2 of the MPI 3.1 standard.
...
///
/// the members specify which group the datatype belongs to according to the
/// type groups specified in Section 5.9.2 of the MPI 3.1 standard.
...
struct mpi_type_traits {
  ...
  /// @brief This member function is only available if \c is_builtin is true.
  /// If this is the case, it returns the \c MPI_Datatype
  /// @returns Constant of type \c MPI_Datatype mapping to type \c T according
  /// the the MPI standard.
...
template <>
struct mpi_type_traits_impl<std::complex<long double>>
  : is_builtin_mpi_type_true {
  static MPI_Datatype data_type() {
...
/// @brief wrapper for \c mpi_type_traits_impl which removes const and volatile
/// qualifiers
template <typename T>
struct mpi_type_traits : mpi_type_traits_impl<std::remove_cv_t<T>> {};
#endif

/// @brief Translate template parameter T to an MPI_Datatype. If no
/// corresponding MPI_Datatype exists, we will create new custom continuous
/// type.
///        Based on
///        https://gist.github.com/2b-t/50d85115db8b12ed263f8231abf07fa2
/// To check if type \c T maps to a builtin \c MPI_Datatype at compile-time, use
/// \c mpi_type_traits.
/// @tparam T The type to translate into a MPI_Datatype.
/// @return The tag identifying the corresponding MPI_Datatype or the newly
/// created type.
...
[[nodiscard]] MPI_Datatype mpi_datatype() KAMPING_NOEXCEPT {
  ...
  static_assert(
    !std::is_pointer_v<T_no_cv>, "MPI does not support pointer types. Why do "
                                 "you want to transfer a pointer over MPI?"
  );
  ...
  static_assert(
    !std::is_void_v<T_no_cv>, "There is no MPI datatype corresponding to void."
  );
