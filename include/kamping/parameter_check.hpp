...
// The following macros look strange since they use a GNU extension that becomes obsolete with
// C++-20. The extension is supported by all major compilers. Semantic is as follows: if the
// variadic parameters of a macro is empty,
//
// , ##__VA_ARGS__
//
// expands to nothing, i.e., the comma vanishes. Otherwise, the comma stays.
// This is required because we overload macros based on their number of arguments; however, the
// preprocessor considers an empty argument to be one argument, i.e., if we have a variadic macro
//
// #define M(...)
//
// and call it as M(), we actually call it with one argument: the empty argument. But since we
// decide what to do with M based on the number of arguments passed to it, we need a way to
// distinguish between "1 empty argument" and "1 actual argument".
//
// Using this trick, "zero arguments" as in "1 argument, but empty" resolves to <empty>, while 1
// actual argument resolves to ", <argument>", i.e., 2 arguments.
//
// However, this leads to the situation where the first argument of this macro is always empty;
// that's why we have a lot of "ignore" parameters in the remaining macros of this file.

/// @brief Wrapper to pass (possibly empty) list of parameter type names as required parameters to
/// \c KAMPING_CHECK_PARAMETERS. Note that this macro only takes the *name* of parameter types,
/// i.e., instead of using `kamping::internal::ParameterType::send_buf`, only pass `send_buf` to
/// this macro.
#define KAMPING_REQUIRED_PARAMETERS(...) , ##__VA_ARGS__

/// @brief Wrapper to pass (possibly empty) list of parameter type names as optional parameters to
/// \c KAMPING_CHECK_PARAMETERS. Note that this macro only takes the *name* of parameter types,
/// i.e., instead of using `kamping::internal::ParameterType::send_buf`, only pass `send_buf` to
/// this macro.
#define KAMPING_OPTIONAL_PARAMETERS(...) , ##__VA_ARGS__

/// @brief Assertion macro that checks if passed parameters are correct, i.e., all parameter types
/// are unique, all required parameters are provided, and no unused parameter is passed. Also checks
/// that all parameter types are r-value references.
///
/// The macro *only* expects the parameter type, i.e., a member name of the
/// `kamping::internal::ParameterType` enum *without the name of the enum*. For instance,
/// ```c++
/// KAMPING_CHECK_PARAMETERS(Args, KAMPING_REQUIRED_PARAMETERS(send_buf, recv_buf),
...
///
/// Note that expanding the macro into a `do { ... } while(false)` pseudo-loop is a common trick to
/// make a macro "act like a statement". Otherwise, it would have surprising effects if the macro is
/// used inside a `if` branch without braces.
///
/// @param args A parameter pack with all parameter types passed to the function. Note that this is
/// only the name of the parameter pack *without trailing `...`*.
/// @param required A list of required parameter type names wrapped in a KAMPING_REQUIRED_PARAMETERS
/// macro.
/// @param optional A list of optional parameter type names wrapped in a KAMPING_OPTIONAL_PARAMETERS
/// macro.
#define KAMPING_CHECK_PARAMETERS(args, required, optional)                                     \
    do {                                                                                       \
        ...
        using parameter_types =                                                                \
            typename kamping::internal::parameters_to_integral_constant<args...>::type;        \
        static_assert(                                                                         \
...
// - KAMPING_PARAMETER_CHECK_HPP_PREFIX_PARAMETERS(...) prepends each argument by
// "kamping::internal::ParameterType::"
// - KAMPING_PARAMETER_CHECK_HPP_ASSERT_REQUIRED_PARAMETERS(args, ...) generates a static assert for
// each of its
//   arguments to ensure that args... contains a parameter of that type.
//
// Since doing something "for each" argument of a variadic macro is unsupported by the preprocessor,
// we use two hacks to implement these macros:
// - Instead of a "for each" loop, we implement macros Xi, for 1 <= i <= 9, such that Xi takes i
// arguments
//   and generates the same code as a "for each" loop would generate for these i arguments.
// - A dispatch macro chooses the right Xi macro depending on the number of arguments passed to the
// dispatch macro.
//
...
// ```
//
// Now, we need a "dispatch" macro `X` that can take 0, 1 or 2 arguments and resolve to `X0`, `X1`
// or `X2`. While we can't make the macro to take between 0 and 2 arguments, we can define it as a
// variadic macro:
//
// ```
// #define X(...) [... magic that expands to X2, X1 or X0 depending on the number of arguments
// passed to X ...]
// ```
//
...
// ```
//
// At least in theory -- if one strictly adheres to the C++ standard, `DISPATCH` actually takes at
// least 4 arguments, since the `...` parameter may not be empty. Thus, in our implementation, we
// pass another dummy argument to the `DISPATCH` invocation, e.g:
// ```
...
// ```
//
// `__VA_ARGS__` expands to whatever arguments we pass to `X`. Thus, if we pass 2 arguments to `X`,
// it also expands to two arguments. If we pass 1 argument to `X`, it expands to 1 argument. Thus,
// we can "move" the correct implementation for `X` to be the 3rd argument passed to `DISPATCH`:
//
// * `X(0, 1)` becomes `DISPATCH(0, 1, X2(0, 1), X1(0, 1), X0, ignore)` becomes `X2(0, 1)`
//
// * `X(0)` becomes `DISPATCH(0, X2(0), X1(0), X0, ignore)` becomes `X1(0)`
//
// Since KAMPING_REQUIRED_PARAMETERS and KAMPING_OPTIONAL_PARAMETERS always resolve to at least one
// argument (see description above), this is sufficient.

// DISPATCH helper macro as described above
#define KAMPING_PARAMETER_CHECK_HPP_SELECT10(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, y, ...) y

// Adds the prefix "kamping::internal::ParameterType::" to each of its arguments (up to 10
// arguments) I.e., turns "send_buf, recv_buf" into "kamping::internal::ParameterType::send_buf,
// kamping::internal::ParameterType::recv_buf"
//
// We do this because we need both versions of the parameter types: to print nice error messages, we
// only need the names of the types without preceeding `kamping::internal::ParameterType::`; to
// implement the checks, we need the actual name of the symbol to generate valid C++ code.
//
...
/// @brief Struct wrapping a check that verifies that no unused parameters are part of the
/// arguments.
///
...
/// std::integral_constant in an \c std::tuple.
/// @tparam Args Arguments passed to the function that calls this check, i.e., the different
/// parameters.
...
struct has_no_unused_parameters {
    ...
    using all_available_parameters =
        decltype(std::tuple_cat(RequiredParametersTuple{}, OptionalParametersTuple{}));

    /// @brief Get total number of different parameters (passed, required, and optional).
    ///
    /// This check works similar to has_all_required_parameters. Here, we "iterate" over all
    /// parameters, i.e., \c RequiredParametersTuple and \c OptionalParametersTuple and check which
    /// parameters are not(!) passed as \c Args. Then, we add this number to the size of Args. If
    /// this number is greater than the total number of (required and optional) parameters, there
    ...
    static constexpr auto number_distinct_parameters(std::index_sequence<Indices...> indices
        ...
        return std::tuple_size_v<decltype(std::tuple_cat(
                   std::conditional_t<
                       !has_parameter_type<
                           std::tuple_element_t<Indices, all_available_parameters>::value,
                           Args...>(),
...
/// kamping::internal::ParameterType passed as template parameter that are extracted as tuple of \c
...
struct parameter_types_to_integral_constants {
    ...
    using type = decltype(std::tuple_cat(
        std::tuple<typename parameter_type_to_integral_constant<ParameterTypes>::type>{}...
    ));
};

/// @brief Wrapper to get a tuple of \c std::integral_constant for each
/// kamping::internal::ParameterType of the parameters.
/// @tparam Parameters Passed parameters for which the kamping::internal::ParameterType are
/// extracted as \c std::integral_constant in a tuple.
template <typename... Parameters>
struct parameters_to_integral_constant {
    /// @brief Type of the tuple.
    using type = decltype(std::tuple_cat(
        std::tuple<
            typename parameter_type_to_integral_constant<Parameters::parameter_type>::type>{}...
    ));
};
