...
/// @brief Each input parameter to one of the \c MPI calls wrapped by KaMPIng needs to has one of
/// the following tags.
///
/// The \c MPI calls wrapped by KaMPIng do not rely on the restricting positional parameter paradigm
/// but use named parameters instead. The ParameterTypes defined in this enum are necessary to
