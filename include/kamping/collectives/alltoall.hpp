...
///
/// This wrapper for \c MPI_Alltoall sends the same amount of data from each rank to each rank. The
/// following buffers are required:
/// - \ref kamping::send_buf() containing the data that is sent to each rank. This buffer has to be
/// the same size at each rank and divisible by the size of the communicator. Each rank receives the
/// same number of elements from this buffer. Rank 0 receives the first `<buffer size>/<communicator
/// size>` elements, rank 1 the next, and so on. See alltoallv() if the amounts differ.
///
/// The following buffers are optional:
/// - \ref kamping::recv_buf() containing a buffer for the output. Afterwards, this buffer will
/// contain the data received as specified for send_buf. The data received from rank 0 comes first,
/// followed by the data received from rank 1, and so on.
...
auto kamping::Communicator::alltoall(Args... args) const {
    ...
    KASSERT(
        ...
        "The number of elements in send_buf is not divisible by the number of ranks in the "
...
///
/// This wrapper for \c MPI_Alltoallv sends the different amounts of data from each rank to each
/// rank. The following buffers are required:
/// - \ref kamping::send_buf() containing the data that is sent to each rank. The size of this
/// buffer has to be at least the sum of the send_counts argument.
...
/// contain the data received as specified for send_buf. The data received from rank 0 comes first,
/// followed by the data received from rank 1, and so on.
/// - \ref kamping::send_displs() containing the offsets of the messages in send_buf. The
/// `send_counts[i]` elements starting at `send_buf[send_displs[i]]` will be sent to rank `i`. If
/// omitted, this is calculated as the exclusive prefix-sum of `send_counts`.
/// - \ref kamping::recv_displs() containing the offsets of the messages in recv_buf. The
/// `recv_counts[i]` elements starting at `recv_buf[recv_displs[i]]` will be received from rank `i`.
/// If omitted, this is calculated as the exclusive prefix-sum of `recv_counts`.
///
/// @tparam Args Automatically deducted template parameters.
/// @param args All required and any number of the optional buffers described above.
/// @return Result type wrapping the output buffer, counts and displacements if not specified as
/// input parameter.
template <typename... Args>
auto kamping::Communicator::alltoallv(Args... args) const {
    ...
    auto const& send_counts =
        internal::select_parameter_type<internal::ParameterType::send_counts>(args...);
    ...
    // Resize recv_buff
    int recv_buf_size =
        *(recv_counts.data() + recv_counts.size() - 1) + // Last element of recv_counts
