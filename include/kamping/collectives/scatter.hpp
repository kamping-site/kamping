...
///
/// This wrapper for \c MPI_Scatter distributes data on the root PE evenly across all PEs in the
/// current communicator.
///
/// The following parameters are mandatory:
/// - \ref kamping::send_buf() containing the data to be evenly distributed across all PEs. The size
/// of this buffer must be divisible by the number of PEs in the current communicator. Non-root PEs
/// can omit a send buffer by passing `kamping::ignore` to \ref kamping::send_buf().
///
/// The following parameters are optional but incur communication overhead if omitted:
/// - \ref kamping::recv_counts() specifying the number of elements sent to each PE. If this
/// parameter is omitted, the number of elements sent to each PE is computed based on the size of
/// the \ref kamping::send_buf() on the root PE and broadcasted to other PEs.
///
/// The following parameters are optional:
/// - \ref kamping::root() specifying the rank of the root PE. If omitted, the default root PE of
/// the communicator is used instead.
/// - \ref kamping::recv_buf() containing the received data. If omitted, a new buffer is allocated
/// and returned.
///
...
auto kamping::Communicator::scatter(Args... args) const {
    ...
    auto&& root_param =
        internal::select_parameter_type_or_default<internal::ParameterType::root, root_param_type>(
            std::tuple(root()),
            args...
        );
    ...
    auto send_buf =
        internal::select_parameter_type<internal::ParameterType::send_buf>(args...).get();
    ...
    KASSERT(
        ...
        "Size of the send buffer (" << send_buf.size()
                                    << ") is not divisible by the number of PEs (" << size()
                                    << ") in the communicator."
    );
