...
///
/// This wrapper for \c MPI_Gather sends the same amount of data from each rank to a root. The
/// following buffers are required:
/// - \ref kamping::send_buf() containing the data that is sent to the root. This buffer has to be
/// the same size at each rank. See TODO gather_v if the amounts differ. The following buffers are
/// optional:
/// - \ref kamping::root() specifying an alternative root. If not present, the default root of the
/// \c Communicator is used, see root().
/// - \ref kamping::recv_buf() containing a buffer for the output. Afterwards, at the root, this
/// buffer will contain all data from all send buffers. At all other ranks, the buffer will have
/// size 0.
...
auto kamping::Communicator::gather(Args... args) const {
  ...
  KASSERT(
    ...
    "All PEs have to send the same number of elements. Use gatherv, if you want to send a "
    "different number of "
    "elements.",
  ...
  auto&& root =
    internal::select_parameter_type_or_default<internal::ParameterType::root, internal::Root>(
      std::tuple(this->root()),
      args...
    );
