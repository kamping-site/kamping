...
///
/// This collective function checks if all ranks have called it with the same value. The result is
/// returned on all ranks.
/// @tparam Value Type of the value to check. Must be comparable with `operator==`.
/// @param value The value of this rank. This value is compared with the ones provided by all other
/// ranks.
...
bool Communicator::is_same_on_all_ranks(Value const& value) const {
  ...
  // std::pair<> is not trivially_copyable and we don't want to forbid comparing std::pair<>s for
  // equality.
  /// @todo How to handle more complex data types, e.g. std::pair<>, user defined classes,
  /// std::vector (here and elsewhere)?
  // static_assert(
