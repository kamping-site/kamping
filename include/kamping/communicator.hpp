...
/// @brief Wrapper for MPI communicator providing access to \ref rank() and \ref size() of the
/// communicator. The \ref Communicator is also access point to all MPI communications provided by
/// KaMPIng.
class Communicator {
public:
    /// @brief Default constructor not specifying any MPI communicator and using \c MPI_COMM_WORLD
    /// by default.
    ...
    /// @param key By default, ranks in the new communicator are determined by the underlying MPI
    /// library (if \c key is 0). Otherwise, ranks are ordered the same way the keys are ordered.
    ...
    [[nodiscard]] int
    convert_rank_to_communicator(int const rank, Communicator const& other_comm) const {
        MPI_Group my_group;
    ...
    [[nodiscard]] int
    convert_rank_from_communicator(int const rank, Communicator const& other_comm) const {
        return other_comm.convert_rank_to_communicator(rank, *this);
    }

    /// @brief Computes a rank that is \c distance ranks away from this MPI thread's current rank
    /// and checks if this is valid rank in this communicator.
    ///
    /// The resulting rank is valid, iff it is at least zero and less than this communicator's size.
    /// The \c distance can be negative. Unlike \ref rank_shifted_cyclic(), this does not guarantee
    /// a valid rank but can indicate if the resulting rank is not valid.
    ...
    /// @brief Computes a rank that is some ranks apart from this MPI thread's rank modulo the
    /// communicator's size.
    ///
    /// When we need to compute a rank that is greater (or smaller) than this communicator's rank,
    /// we can use this function. It computes the rank that is \c distance ranks appart. However,
    /// this function always returns a valid rank, as it computes the rank in a circular fashion,
    /// i.e., \f$ new\_rank=(rank + distance) \% size \f$.
    ...
    /// @brief Checks if a rank is a valid rank for this communicator, i.e., if the rank is in [0,
    /// size).
