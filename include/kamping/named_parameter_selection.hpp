...
/// @brief Base case if there are no parameters: always returns max index
/// indicating that the parameter was not found.
...
/// @brief Returns the Index parameter if the parameter type of Arg matches the
/// requested parameter type. If not, this fails to compile.
///
...
constexpr size_t find_pos() {
    constexpr bool found_arg =
        std::remove_reference_t<Arg>::parameter_type == parameter_type;
...
constexpr size_t find_pos() {
    if constexpr (std::remove_reference_t<Arg>::parameter_type == parameter_type) {
        ...
        // We need to unpack the next two arguments, so we can unambiguously
        // check for the case of a single remaining argument.
...
///
/// @tparam parameter_type The parameter type with which a parameter should be
/// found.
/// @tparam Args All parameter types to be searched for type `parameter_type`.
/// @param args All parameters from which a parameter with the correct type is
/// selected.
...
/// @brief Checks if parameter with requested parameter type exists, if not
/// constructs a default value.
///
/// @tparam parameter_type The parameter type with which a parameter should be
/// found.
/// @tparam Args All parameter types to be searched.
/// @tparam DefaultParameterType The type of the default parameter to be
/// constructed.
/// @tparam DefaultArguments The types of parameters passed to the constructor
/// \c DefaultParameterType.
/// @param default_arguments Tuple of the arguments passed to the constructor of
/// \c DefaultParameterType.
/// @param args All parameters from which a parameter with the correct type is
/// selected.
...
decltype(auto) select_parameter_type_or_default(
    ...
    static_assert(
        ...
        "The default parameter cannot be constructed from the provided "
        "arguments"
    );
    if constexpr (has_parameter_type<parameter_type, Args...>()) {
        constexpr size_t selected_index =
            find_pos<parameter_type, 0, Args...>();
        return std::get<selected_index>(std::forward_as_tuple(args...));
    } else {
        return std::make_from_tuple<DefaultParameterType>(
