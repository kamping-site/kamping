...
//
///// @brief Use this type if one of the template parameters of MPIResult is not used for a specific wrapped \c MPI
/// call.
// struct ResultCategoryNotUsed {};
//
///// @brief Helper for implementing the extract_* functions in \ref MPIResult. Is \c true if the passed buffer type
/// owns
...
//     static constexpr bool has_recv_buffer = internal::has_parameter_type<internal::ParameterType::recv_buf,
//     Args...>();
...
//     template <
//         typename T = std::tuple<Args...>,
//         std::enable_if_t<internal::has_parameter_type_in_tuple<internal::ParameterType::recv_counts, T>(), bool> =
//         true>
...
//     template <
//         typename T = std::tuple<Args...>,
//         std::enable_if_t<internal::has_parameter_type_in_tuple<internal::ParameterType::recv_count, T>(), bool> =
//         true>
...
//     template <
//         typename T = std::tuple<Args...>,
//         std::enable_if_t<internal::has_parameter_type_in_tuple<internal::ParameterType::recv_displs, T>(), bool> =
//         true>
...
//     template <
//         typename T = std::tuple<Args...>,
//         std::enable_if_t<internal::has_parameter_type_in_tuple<internal::ParameterType::send_counts, T>(), bool> =
//         true>
...
//     template <
//         typename T = std::tuple<Args...>,
//         std::enable_if_t<internal::has_parameter_type_in_tuple<internal::ParameterType::send_count, T>(), bool> =
//         true>
...
//     template <
//         typename T = std::tuple<Args...>,
//         std::enable_if_t<internal::has_parameter_type_in_tuple<internal::ParameterType::send_displs, T>(), bool> =
//         true>
...
//     template <
//         typename T = std::tuple<Args...>,
//         std::enable_if_t<internal::has_parameter_type_in_tuple<internal::ParameterType::send_type, T>(), bool> =
//         true>
...
//     template <
//         typename T = std::tuple<Args...>,
//         std::enable_if_t<internal::has_parameter_type_in_tuple<internal::ParameterType::recv_type, T>(), bool> =
//         true>
...
// namespace std {
//
///// @brief Specialization of the std::tuple_size for \ref kamping::MPIResult_. Part of the structured binding
/// machinery.
/////
...
//         return false;
//     } else {
//         return BufferType::is_owning && BufferType::is_out_buffer; ///< Predicate which Head has to fulfill to be
//         kept.
//     }
// }
//
...
// };
//
///// @brief Specialization of template class used to filter a list of (buffer-)types and only keep those whose types
/// meet
...
//         typename FilterOwningOut<Tail...>::type>; ///< A std::tuple<T1, ..., Tn> where T1, ..., Tn are those types
//         among
//                                                   ///< Head, Tail... which fulfill the predicate.
// };
//
///// @brief Specialisation of template class for types stored in a std::tuple<...> that is used to filter these types
/// and
...
///// @returns \c True if the recv (send_recv) buffer is either not mentioned explicitly and no other (owning) out
/// buffers
...
//         return true;
//     } else if constexpr (num_caller_provided_owning_out_buffers == 1 && std::tuple_element_t<0,
//     CallerProvidedOwningOutBuffers>::parameter_type == ParameterType::recv_buf) {
//         return true;
//     } else if constexpr (num_caller_provided_owning_out_buffers == 1 && std::tuple_element_t<0,
//     CallerProvidedOwningOutBuffers>::parameter_type == ParameterType::send_recv_buf) {
//         return true;
//     } else {
//         return false;
//     }
...
///// underlying data (i.e. it is a non-owinig out buffer): In this case recv_buffer is not part of the result object.
/// The
///// \ref kamping::MPIResult_ object stores the buffer to return (owning buffers for which a *_out() named parameter
/// was
...
//     std::tuple_size_v<CallerProvidedOwningOutParameters>; if constexpr (!has_recv_or_send_recv_buf<Buffers...>()) {
...
//                 // object anyway.
//                 return
//                 MPIResult_(construct_buffer_tuple_for_result_object<CallerProvidedOwningOutParameters>(buffers...
//                 ));
//             }
//         }
...
//     NonBlockingResult_(MPIResultType result, RequestDataBuffer request)
...
//         typename NonBlockingResulType_ = NonBlockingResult_<MPIResultType, RequestDataBuffer>,
...
// auto make_nonblocking_result_(Args... args) {
//     auto&& request                 = internal::select_parameter_type<internal::ParameterType::request>(args...);
//     auto   construct_result_object = [&]() {
//         return make_mpi_result_<CallerProvidedArgsInTuple>(std::forward<Args>(args)...);
//     };
//     using result_type = decltype(construct_result_object());
//     if constexpr (is_result_empty_v<result_type>) {
//         return NonBlockingResult_(MPIResult_{std::tuple<>{}}, std::move(request));
//     } else {
//         return NonBlockingResult_(construct_result_object(), std::move(request));
//     }
// }
//
// } // namespace kamping::internal
