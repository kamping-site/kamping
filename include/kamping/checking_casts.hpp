...
/// @brief Checks if an integer value can be safely casted into an integer type
/// To, that is, it lies in the range [min(To), max(To)].
///
...
/// std::numeric_limits<intmax_t>::digits (else) bits. This function includes
/// checks for these two assumptions using static_assert()s.
///
/// @tparam To Type to be casted to.
/// @tparam From Type to be casted from, will be auto inferred.
/// @param value Value you want to cast.
/// @return \c true if value can be safely casted into type To, that is, value
/// is in To's range.
...
///
/// @brief Casts an integer value to the integer type To. If the value is
/// outside To's range, throws an assertion.
///
/// Alternatively, exceptions can be used instead of assertions by using \ref
/// throwing_cast().
///
/// This function works only for integer types which have at most
/// std::numeric_limits<intmax_t>::digits (To and From are signed) or
/// std::numeric_limits<intmax_t>::digits (else) bits. These two assumptions are
/// checked by in_range() using static_assert()s.
///
///
...
///
/// @brief Casts an integer value to the integer type To. If the value is
/// outside To's range, throws an exception.
///
/// Alternatively, assertions can be used instead of exceptions by using \ref
/// asserting_cast().
///
/// This function works only for integer types which have at most
/// std::numeric_limits<intmax_t>::digits (To and From are signed) or
...
constexpr To throwing_cast(From value) {
    THROWING_KASSERT_SPECIFIED(
        in_range<To>(value),
        value << " is not representable by the target type.", std::range_error
    );
    return static_cast<To>(value);
}
