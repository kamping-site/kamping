...
inline constexpr auto make_vbuf = []<typename DataBuffer, typename SizeRange>(DataBuffer&& data, SizeRange&& size_v)
requires DataBufferConcept<DataBuffer> && IntContiguousRange<SizeRange> {
    return std::forward<DataBuffer>(data) | with_size_v(std::forward<SizeRange>(size_v)) | auto_displs() | resize_ext();
};

template <typename T>
inline constexpr auto make_vbuf_vector = []<typename SizeRange>(SizeRange&& size_v)
requires IntContiguousRange<SizeRange> {
    return std::vector<T>{} | with_size_v(std::forward<SizeRange>(size_v)) | auto_displs() | resize_ext();
};
