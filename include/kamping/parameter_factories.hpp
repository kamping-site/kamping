...
/// @brief Creates a user allocated DataBuffer containing the supplied data (a container or a single
/// element)
///
/// Creates a user allocated DataBuffer with the given template parameters and ownership based on
/// whether an rvalue or lvalue reference is passed.
///
...
/// @param data Universal reference to a container or single element holding the data for the
/// buffer.
///
...
auto make_data_buffer(Data&& data) {
  ...
  // Make sure that Data is const, the buffer created is constant (so we don't really remove
  // constness in the return statement below).
...
auto make_data_buffer(std::initializer_list<Data> data) {
  auto data_vec = [&]() {
    if constexpr (std::is_same_v<Data, bool>) {
      ...
      // We only use automatic conversion of bool to kabool for initializer lists, but not for
      // single elements of type bool. The reason for that is, that sometimes single element
      // conversion may not be desired. E.g. consider a gather operation with send_buf :=
      // bool& and recv_buf := Span<bool>, or a bcast with send_recv_buf = bool&
    } else {
...
///
/// @tparam Data Data type for elements in the send buffer. This must be the same type as in the
/// actual send_buf.
...
/// @brief Generates buffer wrapper based on the data in the send buffer, i.e. the underlying
/// storage must contain the data element(s) to send.
///
/// If the underlying container provides \c data(), it is assumed that it is a container and all
/// elements in the container are considered for the operation. In this case, the container has to
/// provide a \c size() member functions and expose the contained \c value_type. If no \c data()
/// member function exists, a single element is wrapped in the send buffer.
/// @tparam Data Data type representing the element(s) to send.
/// @param data Data (either a container which contains the elements or the element directly) to
/// send
...
/// @brief Generates a buffer taking ownership of the data pass to the send buffer as an initializer
/// list.
///
...
/// @brief Generates a buffer wrapper encapsulating a buffer used for sending or receiving based on
/// this processes rank and the root() of the operation. This buffer type may encapsulate const data
/// and in which case it can only be used as the send buffer. For some functions (e.g. bcast), you
/// have to pass a send_recv_buf as the send buffer.
///
/// @tparam Data Data type representing the element(s) to send/receive.
/// @param data Data (either a container which contains the elements or the element directly) to
/// send or the buffer to receive into.
/// @return Object referring to the storage containing the data elements to send / the received
/// elements.
template <typename Data>
auto send_recv_buf(Data&& data) {
  constexpr internal::BufferModifiability modifiability =
    std::is_const_v<std::remove_reference_t<Data>> ? internal::BufferModifiability::constant
                                                   : internal::BufferModifiability::modifiable;
  return internal::make_data_buffer<internal::ParameterType::send_recv_buf, modifiability>(
    std::forward<Data>(data)
  );
}

/// @brief Generates buffer wrapper based on a container for the send counts, i.e. the underlying
/// storage must contain the send counts to each relevant PE.
///
/// The underlying container must provide \c data() and \c size() member functions and expose the
...
/// @brief Generates buffer wrapper based on a container for the recv counts, i.e. the underlying
...
/// @brief Generates buffer wrapper based on a container for the receive counts, i.e. the underlying
/// storage will contained the receive counts when the \c MPI call has been completed. The
/// underlying container must provide a \c data(), \c resize() and \c size() member function and
/// expose the contained \c value_type
...
///
/// This is primarily used by KaMPIng internally because not passing this to a function will have
/// the same effect as passing it.
///
/// @return Wrapper around a new recv_count ouptput integer.
inline auto recv_counts_out(NewContainer<int>&&) {
  // We need this function explicitly, because the user allocated version only takes `int`, not
  // `NewContainer<int>`
  return internal::make_data_buffer<
    internal::ParameterType::recv_counts,
...
/// @brief Generates buffer wrapper based on a container for the send displacements, i.e. the
...
/// @brief Generates a buffer wrapper for the send displacements based on an initializer list, i.e.
/// the send displacements from each relevant PE.
///
/// @tparam Type The type of the initializer list.
...
auto send_displs(std::initializer_list<T> displs) {
  return internal::make_data_buffer<
    internal::ParameterType::send_displs,
    internal::BufferModifiability::constant>(std::move(displs));
}

/// @brief Generates buffer wrapper based on a container for the recv displacements, i.e. the
...
/// @brief Generates a buffer wrapper for the receive displacements based on an initializer list,
/// i.e. the receive displacements from each relevant PE.
///
/// @tparam Type The type of the initializer list.
...
/// @brief Generates buffer wrapper based on a container for the receive buffer, i.e. the underlying
/// storage will contained the received elements when the \c MPI call has been completed. The
/// underlying container must provide a \c data(), \c resize() and \c size() member function and
/// expose the contained \c value_type
...
/// @brief Generates buffer wrapper based on a container for the send displacements, i.e. the
/// underlying storage will contained the send displacements when the \c MPI call has been
/// completed. The underlying container must provide a \c data(), \c resize() and \c size() member
...
/// @brief Generates buffer wrapper based on a container for the receive displacements, i.e. the
/// underlying storage will contained the receive displacements when the \c MPI call has been
/// completed. The underlying container must provide a \c data(), \c resize() and \c size() member
/// function and expose the contained \c value_type
...
/// @brief Generates an object encapsulating the rank of the root PE. This is useful for \c MPI
/// functions like \c MPI_Gather.
///
...
///     undefined_commutative is only supported for builtin operations. This is used to streamline
///     the interface so that the use does not have to provide commutativity info when the operation
///     is builtin.
