...
/// @brief Creates a user allocated DataBuffer containing the supplied data (a
/// container or a single element)
///
/// Creates a user allocated DataBuffer with the given template parameters and
/// ownership based on whether an rvalue or lvalue reference is passed.
///
...
/// @param data Universal reference to a container or single element holding the
/// data for the buffer.
///
...
auto make_data_buffer(Data&& data) {
    ...
    // Make sure that Data is const, the buffer created is constant (so we don't
    // really remove constness in the return statement below).
    constexpr bool is_const_data_type =
        std::is_const_v<std::remove_reference_t<Data>>;
    constexpr bool is_const_buffer =
        modifiability == BufferModifiability::constant;
...
/// @brief Creates an owning DataBuffer containing the supplied data in a
/// std::vector.
///
/// Creates an owning DataBuffer with the given template parameters.
///
/// An initializer list of type \c bool will be converted to a \c
/// std::vector<kamping::kabool>.
///
/// @tparam parameter_type parameter type represented by this buffer.
/// @tparam modifiability `modifiable` if a KaMPIng operation is allowed to
/// modify the underlying container. `constant` otherwise.
...
auto make_data_buffer(std::initializer_list<Data> data) {
    ...
    return DataBuffer<
        decltype(data_vec), parameter_type, modifiability,
...
/// @brief Tag for parameters that can be omitted on some PEs (e.g., root PE, or
/// non-root PEs).
template <typename T>
...
///
/// This is useful for operations where a send_buf is required on some PEs, such
/// as the root PE, but not all PEs that participate in the collective
/// communication.
///
/// @tparam Data Data type for elements in the send buffer. This must be the
/// same type as in the actual send_buf.
/// @param ignore Tag parameter for overload dispatching, pass in
/// `kamping::ignore<Data>`.
...
/// @brief Generates buffer wrapper based on the data in the send buffer, i.e.
/// the underlying storage must contain the data element(s) to send.
///
/// If the underlying container provides \c data(), it is assumed that it is a
/// container and all elements in the container are considered for the
/// operation. In this case, the container has to provide a \c size() member
/// functions and expose the contained \c value_type. If no \c data() member
/// function exists, a single element is wrapped in the send buffer.
/// @tparam Data Data type representing the element(s) to send.
/// @param data Data (either a container which contains the elements or the
/// element directly) to send
/// @return Object referring to the storage containing the data elements to
/// send.
template <typename Data>
...
/// receiving based on this processes rank and the root() of the operation. This
/// buffer type may encapsulate const data and in which case it can only be used
/// as the send buffer. For some functions (e.g. bcast), you have to pass a
/// send_recv_buf as the send buffer.
///
/// @tparam Data Data type representing the element(s) to send/receive.
/// @param data Data (either a container which contains the elements or the
/// element directly) to send or the buffer to receive into.
...
/// i.e. the underlying storage must contain the send counts to each relevant
/// PE.
///
/// The underlying container must provide \c data() and \c size() member
/// functions and expose the contained \c value_type
...
/// @brief Generates a buffer wrapper for the send counts based on an
/// initializer list, i.e. the send counts to each relevant PE.
///
...
/// @brief Generates buffer wrapper based on a container for the recv counts,
/// i.e. the underlying storage must contain the recv counts from each relevant
/// PE.
///
/// The underlying container must provide \c data() and \c size() member
/// functions and expose the contained \c value_type
...
/// @brief Generates a buffer wrapper for the recv counts based on an
/// initializer list, i.e. the recv counts from each relevant PE.
///
/// @tparam Type The type of the initializer list.
...
/// KaMPIng.
///
/// This is primarily used by KaMPIng internally because not passing this to a
...
/// displacements, i.e. the underlying storage must contain the send
/// displacements to each relevant PE.
///
/// The underlying container must provide \c data() and \c size() member
/// functions and expose the contained \c value_type
...
/// @brief Generates a buffer wrapper for the send displacements based on an
/// initializer list, i.e. the send displacements from each relevant PE.
///
/// @tparam Type The type of the initializer list.
...
/// @brief Generates buffer wrapper based on a container for the recv
/// displacements, i.e. the underlying storage must contain the recv
/// displacements from each relevant PE.
///
/// The underlying container must provide \c data() and \c size() member
/// functions and expose the contained \c value_type
...
/// @brief Generates a buffer wrapper for the receive displacements based on an
/// initializer list, i.e. the receive displacements from each relevant PE.
///
/// @tparam Type The type of the initializer list.
...
/// @brief Generates buffer wrapper based on a container for the receive buffer,
/// i.e. the underlying storage will contained the received elements when the \c
...
/// @brief Generates buffer wrapper based on a container for the send
/// displacements, i.e. the underlying storage will contained the send
...
/// @brief Generates an object encapsulating the rank of the root PE. This is
/// useful for \c MPI functions like \c MPI_Gather.
///
...
///     undefined_commutative is only supported for builtin operations. This is
