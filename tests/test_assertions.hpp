...
/// @file
/// @brief Redefines the KASSERT macro such that assertions throw exceptions instead of aborting the
/// process. This is needed because GoogleTest does not support death tests in a multithreaded
/// program (MPI spawns multiple threads).
///
/// *NOTE THAT THIS HEADER MUST BE INCLUDED BEFORE ANY OTHER KAMPING HEADERS* since it redefines the
/// KASSERT macro. This must happen before the preprocessor substitutes the macro invocations.
#pragma once
    ...
    if constexpr (kassert::internal::assertion_enabled(level)) {                        \
      if (!(expression)) {                                                              \
        throw kamping::testing::KassertTestingException(                                \
          (kassert::internal::RrefOStringstreamLogger{std::ostringstream{}} << message) \
            .stream()                                                                   \
            .str()                                                                      \
        );                                                                              \
      }                                                                                 \
...
// Makros to test for failed KASSERTs
  ...
  // EXPECT that any KASSERT assertion failed. The failure message is ignored since EXPECT_THROW
  // does not support one.
  #define EXPECT_KASSERT_FAILS(code, failure_message) \
    EXPECT_THROW({ code; }, ::kamping::testing::KassertTestingException);

  // ASSERT that any KASSERT assertion failed. The failure message is ignored since ASSERT_THROW
  // does not support one.
