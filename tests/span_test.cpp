...
TEST(SpanTest, basic_functionality) {
  ...
  static_assert(
    ...
    "Member data() of internal::Span<T const *, size_t> does not return a "
    "pointer."
  );
  static_assert(
    std::is_const_v<std::remove_pointer_t<decltype(const_int_span.data())>>,
    "Member data() of internal::Span<T const *, size_t> does not return a "
    "pointer pointing to const memory."
  );
  static_assert(
    !std::is_const_v<std::remove_pointer_t<decltype(int_span.data())>>,
    "Member data() of internal::Span<T*, size_t> does return a pointer "
    "pointing to const memory."
  );
  ...
  static_assert(
    ...
    "Member pointer of internal::Span<T*, difference_t> does not match the "
    "element type of the underlying "
    "container."
  );
  static_assert(
    std::is_same_v<
  ...
  static_assert(
    ...
    "Member reference of internal::Span<T*, difference_t> does not match the "
    "element type of the underlying "
    "container."
  );
  static_assert(
