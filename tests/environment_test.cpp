...
static MPI_Datatype last_commited_type = MPI_DATATYPE_NULL;

int MPI_Type_commit(MPI_Datatype* type) {
    last_commited_type = *type;
    return PMPI_Type_commit(type);
}

TEST_F(EnvironmentTest, commit_test) {
    last_commited_type = MPI_DATATYPE_NULL;
    Environment<kamping::InitMPIMode::NoInitFinalize> env;
    EXPECT_TRUE(freed_types.empty());
    MPI_Datatype type;
    MPI_Type_contiguous(1, MPI_CHAR, &type);
    EXPECT_EQ(last_commited_type, MPI_DATATYPE_NULL);
    env.commit(type);
