...
TEST(CheckingCastTest, asserting_cast) {
    ...
    // This works by exiting with a 0 code after the expression and letting gtest check whether that
    // exit occurred. From
    ...
    if constexpr (KASSERT_ASSERTION_LEVEL >= kamping::assert::normal) {
        // According to the googletest documentation, throwing an exception is not considered a
        // death. This ASSERT should therefore only succeed if an assert() fails, not if an
        // exception is thrown.
...
///
/// @brief Checks if a functions fails with a std::range_error exception if exception mode is
/// enabled and an assertion when exception mode is disabled using google test.
///
/// @tparam Lambda Function to check for failures.
/// @param callable Function to check for failures.
/// @param what Substring that should be contained in the output of what() of the thrown exception.
/// Ignored if empty.
///
