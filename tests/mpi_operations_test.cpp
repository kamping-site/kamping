...
constexpr T identity_op_value(Op operation, T const& value) {
    auto const identity =
        kamping::internal::mpi_operation_traits<Op, T>::identity;
...
TYPED_TEST(TypedOperationsTest, test_builtin_operations) {
    ...
    if constexpr (kamping::mpi_type_traits<T>::category == kamping::TypeCategory::integer || kamping::mpi_type_traits<T>::category == kamping::TypeCategory::floating) {
        ...
        EXPECT_EQ(
            (mpi_operation_traits<kamping::ops::max<T>, T>::op()),
            MPI_MAX
        );
        EXPECT_TRUE((mpi_operation_traits<kamping::ops::max<>, T>::is_builtin));
        EXPECT_EQ(
            (mpi_operation_traits<kamping::ops::max<>, T>::op()),
            MPI_MAX
        );
        EXPECT_FALSE(
            (mpi_operation_traits<kamping::ops::max<std::complex<int>>, T>::
                 is_builtin)
        );
        for (auto& value: some_values_v<T>) {
            EXPECT_EQ(
                value,
                (value_op_identity(kamping::ops::max<T>{}, value))
            );
            EXPECT_EQ(
                value,
                (identity_op_value(kamping::ops::max<T>{}, value))
            );
        }
        ...
        EXPECT_EQ(
            (mpi_operation_traits<kamping::ops::min<T>, T>::op()),
            MPI_MIN
        );
        EXPECT_TRUE((mpi_operation_traits<kamping::ops::min<>, T>::is_builtin));
        EXPECT_EQ(
            (mpi_operation_traits<kamping::ops::min<>, T>::op()),
            MPI_MIN
        );
        EXPECT_FALSE(
            (mpi_operation_traits<kamping::ops::min<std::complex<int>>, T>::
                 is_builtin)
        );
        for (auto& value: some_values_v<T>) {
            EXPECT_EQ(
                value,
                (value_op_identity(kamping::ops::min<T>{}, value))
            );
            EXPECT_EQ(
                value,
                (identity_op_value(kamping::ops::min<T>{}, value))
            );
        }
    }
    ...
    if constexpr (kamping::mpi_type_traits<T>::category == kamping::TypeCategory::integer || kamping::mpi_type_traits<T>::category == kamping::TypeCategory::floating || kamping::mpi_type_traits<T>::category == kamping::TypeCategory::complex) {
        ...
        EXPECT_EQ(
            (mpi_operation_traits<kamping::ops::plus<T>, T>::op()),
            MPI_SUM
        );
        EXPECT_TRUE((mpi_operation_traits<kamping::ops::plus<>, T>::is_builtin)
        );
        EXPECT_EQ(
            (mpi_operation_traits<kamping::ops::plus<>, T>::op()),
            MPI_SUM
        );
        ...
        EXPECT_FALSE(
            (mpi_operation_traits<kamping::ops::plus<std::complex<int>>, T>::
                 is_builtin)
        );
        for (auto& value: some_values_v<T>) {
            EXPECT_EQ(
                value,
                (value_op_identity(kamping::ops::plus<T>{}, value))
            );
            EXPECT_EQ(
                value,
                (identity_op_value(kamping::ops::plus<T>{}, value))
            );
        }
        ...
        EXPECT_TRUE(
            (mpi_operation_traits<kamping::ops::multiplies<>, T>::is_builtin)
        );
        EXPECT_EQ(
        ...
        EXPECT_FALSE((mpi_operation_traits<
                      kamping::ops::multiplies<std::complex<int>>,
                      T>::is_builtin));
        for (auto& value: some_values_v<T>) {
            EXPECT_EQ(
                value,
    ...
    if constexpr (kamping::mpi_type_traits<T>::category == kamping::TypeCategory::integer || kamping::mpi_type_traits<T>::category == kamping::TypeCategory::logical) {
        ...
        EXPECT_EQ(
            (mpi_operation_traits<std::logical_and<>, T>::op()),
            MPI_LAND
        );
        EXPECT_FALSE((mpi_operation_traits<
                      kamping::ops::logical_and<std::complex<int>>,
                      T>::is_builtin));
        for (auto& value: some_values_v<T>) {
            EXPECT_EQ(
                static_cast<bool>(value),
                (value_op_identity(kamping::ops::logical_and<T>{}, value))
            );
            EXPECT_EQ(
                static_cast<bool>(value),
                (identity_op_value(kamping::ops::logical_and<T>{}, value))
            );
        }
        ...
        EXPECT_TRUE(
            (mpi_operation_traits<kamping::ops::logical_or<>, T>::is_builtin)
        );
        EXPECT_EQ(
        ...
        EXPECT_FALSE((mpi_operation_traits<
                      kamping::ops::logical_or<std::complex<int>>,
                      T>::is_builtin));
        for (auto& value: some_values_v<T>) {
            EXPECT_EQ(
                static_cast<bool>(value),
                (value_op_identity(kamping::ops::logical_or<T>{}, value))
            );
            EXPECT_EQ(
                static_cast<bool>(value),
                (identity_op_value(kamping::ops::logical_or<T>{}, value))
            );
        }
        ...
        EXPECT_FALSE((mpi_operation_traits<
                      kamping::ops::logical_xor<std::complex<int>>,
                      T>::is_builtin));
        for (auto& value: some_values_v<T>) {
            EXPECT_EQ(
                static_cast<bool>(value),
                (value_op_identity(kamping::ops::logical_xor<T>{}, value))
            );
            EXPECT_EQ(
                static_cast<bool>(value),
                (identity_op_value(kamping::ops::logical_xor<T>{}, value))
            );
        }
    }
    ...
    if constexpr (kamping::mpi_type_traits<T>::category == kamping::TypeCategory::integer || kamping::mpi_type_traits<T>::category == kamping::TypeCategory::byte) {
        EXPECT_TRUE(
            (mpi_operation_traits<kamping::ops::bit_and<T>, T>::is_builtin)
        );
        EXPECT_EQ(
        ...
        EXPECT_TRUE(
            (mpi_operation_traits<kamping::ops::bit_and<>, T>::is_builtin)
        );
        EXPECT_EQ(
        ...
        for (auto& value: some_values_v<T>) {
            EXPECT_EQ(
                value,
                (value_op_identity(kamping::ops::bit_and<T>{}, value))
            );
            EXPECT_EQ(
                value,
                (identity_op_value(kamping::ops::bit_and<T>{}, value))
            );
        }

        EXPECT_TRUE(
            (mpi_operation_traits<kamping::ops::bit_or<T>, T>::is_builtin)
        );
        EXPECT_EQ(
        ...
        EXPECT_EQ(
            (mpi_operation_traits<kamping::ops::bit_or<>, T>::op()),
            MPI_BOR
        );
        ...
        for (auto& value: some_values_v<T>) {
            EXPECT_EQ(
                value,
                (value_op_identity(kamping::ops::bit_or<T>{}, value))
            );
            EXPECT_EQ(
                value,
                (identity_op_value(kamping::ops::bit_or<T>{}, value))
            );
        }

        EXPECT_TRUE(
            (mpi_operation_traits<kamping::ops::bit_xor<T>, T>::is_builtin)
        );
        EXPECT_EQ(
        ...
        EXPECT_TRUE(
            (mpi_operation_traits<kamping::ops::bit_xor<>, T>::is_builtin)
        );
        EXPECT_EQ(
        ...
        for (auto& value: some_values_v<T>) {
            EXPECT_EQ(
                value,
                (value_op_identity(kamping::ops::bit_xor<T>{}, value))
            );
            EXPECT_EQ(
                value,
                (identity_op_value(kamping::ops::bit_xor<T>{}, value))
            );
        }
    }
}
...
TYPED_TEST(TypedOperationsTest, user_defined_operation_is_not_builtin_lambda) {
    ...
    EXPECT_FALSE(
        (kamping::internal::mpi_operation_traits<decltype(op), T>::is_builtin)
    );
}

TYPED_TEST(
    TypedOperationsTest, user_defined_operation_is_not_builtin_function_object
) {
    using T = typename TestFixture::operation_type;
    struct MyOperation {
    ...
    EXPECT_FALSE(
        (kamping::internal::mpi_operation_traits<MyOperation, T>::is_builtin)
    );
}

TYPED_TEST(
    TypedOperationsTest,
    user_defined_operation_is_not_builtin_unsupported_stl_operation
) {
    using T = typename TestFixture::operation_type;
    EXPECT_FALSE(
        (kamping::internal::mpi_operation_traits<std::minus<>, T>::is_builtin)
    );
    EXPECT_FALSE(
        (kamping::internal::mpi_operation_traits<std::divides<>, T>::is_builtin)
    );
}
...
TEST(OperationsTest, builtin_operations_on_unsupported_type) {
    ...
    EXPECT_FALSE(
        (kamping::internal::
             mpi_operation_traits<kamping::ops::max<>, DummyType>::is_builtin)
    );
    ...
    EXPECT_FALSE(
        (kamping::internal::
             mpi_operation_traits<kamping::ops::min<>, DummyType>::is_builtin)
    );
    EXPECT_FALSE((kamping::internal::mpi_operation_traits<
                  kamping::ops::min<>,
                  std::complex<double>>::is_builtin));
    ...
    EXPECT_FALSE(
        (kamping::internal::
             mpi_operation_traits<kamping::ops::plus<>, DummyType>::is_builtin)
    );
    EXPECT_FALSE(
        (kamping::internal::mpi_operation_traits<kamping::ops::plus<>, bool>::
             is_builtin)
    );
    EXPECT_FALSE((kamping::internal::mpi_operation_traits<
                  kamping::ops::multiplies<>,
                  DummyType>::is_builtin));
    EXPECT_FALSE(
        (kamping::internal::
             mpi_operation_traits<kamping::ops::multiplies<>, bool>::is_builtin)
    );
    // logical operations
    EXPECT_FALSE((kamping::internal::mpi_operation_traits<
                  kamping::ops::logical_and<>,
                  DummyType>::is_builtin));
    EXPECT_FALSE((kamping::internal::mpi_operation_traits<
                  kamping::ops::logical_and<>,
                  double>::is_builtin));
    EXPECT_FALSE((kamping::internal::mpi_operation_traits<
                  kamping::ops::logical_or<>,
                  DummyType>::is_builtin));
    EXPECT_FALSE((
        kamping::internal::
    ...
    EXPECT_FALSE(
        (kamping::internal::
             mpi_operation_traits<kamping::ops::bit_and<>, double>::is_builtin)
    );
    EXPECT_FALSE((
        kamping::internal::
    ...
    EXPECT_FALSE(
        (kamping::internal::
             mpi_operation_traits<kamping::ops::bit_or<>, double>::is_builtin)
    );
    EXPECT_FALSE((
        kamping::internal::
    ...
    EXPECT_FALSE(
        (kamping::internal::
             mpi_operation_traits<kamping::ops::bit_xor<>, double>::is_builtin)
    );
}
