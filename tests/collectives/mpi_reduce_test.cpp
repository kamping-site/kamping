...
TEST(ReduceTest, reduce_no_receive_buffer_bool) {
  ...
  auto result =
    comm.reduce(send_buf(input), op(ops::logical_or<>{})).extract_recv_buffer();
...
TEST(ReduceTest, reduce_no_receive_buffer_kabool_custom_operation) {
  ...
  // test that we can use a operation defined on bool even though wrap them as
  // kabool
  auto my_or = [&](bool lhs, bool rhs) {
    return lhs || rhs;
  };
  auto result =
    comm.reduce(send_buf(input), op(my_or, commutative)).extract_recv_buffer();
...
TEST(
  ReduceTest, reduce_single_element_initializer_list_bool_no_receive_buffer
) {
  Communicator comm;
...
TEST(ReduceTest, reduce_builtin_op_on_non_builtin_type) {
  ...
  auto               result =
    comm
      .reduce(send_buf(input), op(kamping::ops::plus<>{}, kamping::commutative))
      .extract_recv_buffer();
  if (comm.is_root()) {
    EXPECT_EQ(result.size(), 2);
...
TEST(ReduceTest, reduce_custom_operation_on_builtin_type) {
  ...
  // use function ptr
  auto result =
    comm.reduce(send_buf(input), op(add_plus_42_function, kamping::commutative))
      .extract_recv_buffer();
  ...
  // use lambda
  result =
    comm.reduce(send_buf(input), op(add_plus_42_lambda, kamping::commutative))
      .extract_recv_buffer();
  ...
  result =
    comm
      .reduce(
        send_buf(input),
        op(
          [](auto const& lhs, auto const& rhs) { return lhs + rhs + 42; },
          kamping::commutative
        )
      )
      .extract_recv_buffer();
  ...
  result = comm.reduce(send_buf(input), op(MySum42{}, kamping::commutative))
             .extract_recv_buffer();
...
TEST(ReduceTest, reduce_custom_operation_on_builtin_type_non_commutative) {
  ...
  auto result =
    comm.reduce(send_buf(input), op(get_right, kamping::non_commutative))
      .extract_recv_buffer();
  ...
  struct Aggregate {
    ...
    bool operator==(Aggregate const& rhs) const {
      return this->min == rhs.min && this->max == rhs.max
             && this->flag == rhs.flag && this->sum == rhs.sum;
    }
  };
...
//             comm.reduce(send_buf(value), op(kamping::ops::plus<>{}),
//             root(comm.rank())), "Root has to be the same on all ranks.");
//     }
// }
