...
TEST(ReduceTest, reduce_no_receive_buffer) {
    ...
    for (size_t i = 0; i < comm.size(); ++i) {
        result =
          comm.reduce(send_buf(input), op(kamping::ops::plus<>{}), root(i))
            .extract_recv_buffer();
        if (comm.rank() == i) {
            EXPECT_EQ(comm.root(), comm.size() - 1);
            EXPECT_EQ(result.size(), 2);
...
TEST(ReduceTest, reduce_no_receive_buffer_kabool_custom_operation) {
    ...
    auto result = comm.reduce(send_buf(input), op(my_or, commutative))
                    .extract_recv_buffer();
...
TEST(ReduceTest, reduce_custom_operation_on_builtin_type) {
    ...
    auto result =
      comm
        .reduce(send_buf(input), op(add_plus_42_function, kamping::commutative))
        .extract_recv_buffer();
