...
TEST(ReduceTest, reduce_no_receive_buffer) {
    ...
    for (size_t i = 0; i < comm.size(); ++i) {
        result =
            comm.reduce(send_buf(input), op(kamping::ops::plus<>{}), root(i)).extract_recv_buffer();
        if (comm.rank() == i) {
            EXPECT_EQ(comm.root(), comm.size() - 1);
            EXPECT_EQ(result.size(), 2);
            EXPECT_EQ(result, expected_result);
...
TEST(ReduceTest, reduce_single_element_initializer_list_bool_no_receive_buffer) {
    ...
    // reduce does not support single element bool when no recv_buf is specified, because the
    // default would be std::vector<bool>, which is not supported
    auto result = comm.reduce(send_buf({input}), op(ops::logical_or<>{})).extract_recv_buffer();
...
TEST(ReduceTest, reduce_with_receive_buffer_on_root) {
    ...
    if (comm.is_root()) {
        ...
        auto result =
            comm.reduce(send_buf(input), op(kamping::ops::plus<>{})).extract_recv_buffer();
        EXPECT_EQ(result.size(), 0);
    }
}
...
TEST(ReduceTest, reduce_builtin_op_on_non_builtin_type) {
    ...
    auto result = comm.reduce(send_buf(input), op(kamping::ops::plus<>{}, kamping::commutative))
                      .extract_recv_buffer();
    if (comm.is_root()) {
        EXPECT_EQ(result.size(), 2);
...
TEST(ReduceTest, reduce_custom_operation_on_builtin_type) {
    ...
    // use function ptr
    auto result = comm.reduce(send_buf(input), op(add_plus_42_function, kamping::commutative))
                      .extract_recv_buffer();
    ...
    // use lambda
    result = comm.reduce(send_buf(input), op(add_plus_42_lambda, kamping::commutative))
                 .extract_recv_buffer();
    ...
    result = comm.reduce(
                     send_buf(input),
                     op([](auto const& lhs, auto const& rhs) { return lhs + rhs + 42; },
                        kamping::commutative)
    )
                 .extract_recv_buffer();
    ...
    result =
        comm.reduce(send_buf(input), op(MySum42{}, kamping::commutative)).extract_recv_buffer();
...
TEST(ReduceTest, reduce_custom_operation_on_builtin_type_non_commutative) {
    ...
    auto result =
        comm.reduce(send_buf(input), op(get_right, kamping::non_commutative)).extract_recv_buffer();
...
TEST(ReduceTest, reduce_custom_operation_on_custom_type) {
    ...
    struct Aggregate {
        ...
        bool operator==(Aggregate const& rhs) const {
            return this->min == rhs.min && this->max == rhs.max && this->flag == rhs.flag
                   && this->sum == rhs.sum;
        }
    };
    ...
    auto result =
        comm.reduce(send_buf(input), op(my_op, kamping::commutative)).extract_recv_buffer();
