...
TEST(
  GatherTest, gather_single_element_initializer_list_bool_no_receive_buffer
) {
  Communicator comm;
  // gather does not support single element bool when specifying no
  // recv_buffer, because the default receive buffer is std::vector<bool>,
...
//     comm.size() > 1) {
//         EXPECT_KASSERT_FAILS(comm.gather(send_buf(value), root(comm.rank())),
