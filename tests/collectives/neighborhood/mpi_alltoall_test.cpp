...
TEST(NeighborhoodAlltoallTest, single_element_no_receive_buffer_for_edges_to_predecessor_successor) {
    ...
    const std::vector<size_t> input = edges;
...
TEST(NeighborhoodAlltoallTest, single_element_no_receive_buffer_for_edges_to_successor) {
    ...
    const std::vector<size_t> input{comm.rank()};
...
TEST(NeighborhoodAlltoallTest, single_element_no_receive_buffer_for_multi_edges_to_successor) {
    ...
    const std::vector<size_t> input(edge_multiplicity, comm.rank());
...
//     // A rank sends two integers to each other rank. A rank receives two integers into a custom datatype with can
//     store
...
//     std::vector<int> expected_result(comm.size() * 2); // the buffer will be resized to only hold the received
//     elements for (size_t i = 0; i < comm.size(); ++i) {
//         expected_result[i * 2]     = static_cast<int>(i);
//         expected_result[i * 2 + 1] = static_cast<int>(i);
//     }
//     EXPECT_EQ(input, expected_result);
