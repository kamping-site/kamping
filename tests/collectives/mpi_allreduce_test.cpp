...
TEST(AllreduceTest, allreduce_custom_operation_on_builtin_type) {
    ...
    { // use function object
        ...
        auto result =
          comm.allreduce(send_buf(input), op(MySum42{}, kamping::commutative))
            .extract_recv_buffer();
...
TEST(
  AllreduceTest, allreduce_custom_operation_on_builtin_type_non_commutative
) {
    Communicator comm;
    ...
    auto result =
      comm.allreduce(send_buf(input), op(get_right, kamping::non_commutative))
        .extract_recv_buffer();
...
TEST(AllreduceTest, allreduce_custom_operation_on_custom_type) {
    ...
    auto result =
      comm.allreduce(send_buf(input), op(my_op, kamping::commutative))
        .extract_recv_buffer();
