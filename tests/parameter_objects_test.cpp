...
TEST(ContainerBasedConstBufferTest, get_basics) {
  ...
  static_assert(std::is_same_v<
                decltype(buffer_based_on_int_vector.data()),
                const int*>);

  EXPECT_EQ(
    buffer_based_on_const_int_vector.get().size(),
    int_vec_const.size()
  );
  EXPECT_EQ(
    buffer_based_on_const_int_vector.get().data(),
    int_vec_const.data()
  );
  static_assert(std::is_same_v<
...
TEST(ContainerBasedOwningBufferTest, get_basics) {
  ...
  static_assert(std::is_same_v<
                decltype(buffer_based_on_moved_vector.data()),
                int const*>);
  ...
  static_assert(std::is_same_v<
                decltype(buffer_based_on_rvalue_vector.data()),
                int const*>);
...
TEST(SingleElementConstBufferTest, get_basics) {
  ...
  static_assert(std::
                  is_same_v<decltype(int_buffer)::value_type, decltype(value)>);
}
...
TEST(LibAllocatedContainerBasedBufferTest, prevent_usage_after_extraction) {
  ...
  EXPECT_KASSERT_FAILS(
    buffer.get(),
    "Cannot get a buffer that has already been extracted."
  );
  EXPECT_KASSERT_FAILS(
    buffer.data(),
