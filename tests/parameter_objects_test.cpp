...
TEST(IsSpecializationTest, is_specialization_basics) {
    ...
    EXPECT_TRUE((kamping::internal::
                     is_specialization<std::vector<bool>, std::vector>::value));
    ...
    EXPECT_FALSE((kamping::internal::
                      is_specialization<std::array<int, 2>, std::vector>::value)
    );
    EXPECT_FALSE((kamping::internal::
                      is_specialization<std::deque<int>, std::vector>::value));
...
TEST(SingleElementConstBufferTest, get_basics) {
    ...
    static_assert(std::is_same_v<
                  decltype(int_buffer)::value_type,
                  decltype(value)>);
}
