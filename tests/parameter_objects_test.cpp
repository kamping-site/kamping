...
TEST(HasDataMemberTest, has_data_member_basics) {
  ...
  EXPECT_TRUE(
    (kamping::internal::has_data_member_v<std::vector<double, testing::CustomAllocator<double>>>)
  );
...
TEST(ContainerBasedConstBufferTest, get_basics) {
  ...
  static_assert(std::
                  is_same_v<decltype(buffer_based_on_const_int_vector.get().data()), const int*>);
