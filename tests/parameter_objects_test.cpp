...
TEST(IsSpecializationTest, is_specialization_basics) {
    ...
    EXPECT_TRUE((kamping::internal::is_specialization<
                 std::vector<bool>, std::vector>::value));
    EXPECT_TRUE(
    ...
    EXPECT_FALSE((kamping::internal::is_specialization<
                  std::array<int, 2>, std::vector>::value));
    EXPECT_FALSE((
...
TEST(ContainerBasedConstBufferTest, get_basics) {
    ...
    static_assert(std::is_same_v<
                  decltype(buffer_based_on_int_vector.get().data()),
                  const int*>);
    ...
    static_assert(std::is_same_v<
                  decltype(buffer_based_on_const_int_vector.data()),
                  const int*>);
}
...
TEST(ContainerBasedOwningBufferTest, get_containers_other_than_vector) {
    ...
    EXPECT_EQ(
      std::string(
        ...
        buffer_based_on_string.get().data()
          + buffer_based_on_string.get().size()
      ),
      expected
    );
    {
...
TEST(ContainerBasedOwningBufferTest, move_constructor_is_enabled) {
    ...
    ContainerBasedOwningBuffer<std::vector<int>, ptype> buffer2(
      std::move(buffer1)
    );
...
TEST(LibAllocatedContainerBasedBufferTest, resize_and_data_extract_basics) {
    ...
    std::vector<int> underlying_container =
      buffer_based_on_int_vector.extract();
...
TEST(
  ...
  prevent_usage_after_extraction_via_mpi_result
    ...
    LibAllocatedContainerBasedBuffer<
      ...
      recv_displs;
    LibAllocatedContainerBasedBuffer<
      std::vector<int>, ParameterType::send_displs>
      send_displs;
