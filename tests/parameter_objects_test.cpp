...
TEST(HasDataMemberTest, has_data_member_basics) {
  ...
  EXPECT_TRUE((kamping::internal::has_data_member_v<
               std::vector<double, testing::CustomAllocator<double>>>));
  ...
  // EXPECT_FALSE((kamping::internal::has_data_member_v<std::vector<bool,
  // testing::CustomAllocator<bool>>>));
}

TEST(IsSpecializationTest, is_specialization_basics) {
  EXPECT_TRUE(
    (kamping::internal::is_specialization<std::vector<int>, std::vector>::value)
  );
  EXPECT_TRUE((
    kamping::internal::is_specialization<std::vector<bool>, std::vector>::value
  ));
  EXPECT_TRUE(
    (kamping::internal::is_specialization<
      std::vector<int, testing::CustomAllocator<int>>, std::vector>::value)
  );
  EXPECT_TRUE((
    kamping::internal::is_specialization<
  ...
  EXPECT_TRUE(
    (kamping::internal::is_specialization<std::deque<int>, std::deque>::value)
  );

  EXPECT_FALSE((
    kamping::internal::is_specialization<std::array<int, 2>, std::vector>::value
  ));
  EXPECT_FALSE(
    (kamping::internal::is_specialization<std::deque<int>, std::vector>::value)
  );
...
TEST(IsVectorBoolTest, is_vector_bool_basics) {
  ...
  EXPECT_TRUE((kamping::internal::is_vector_bool_v<
               std::vector<bool, testing::CustomAllocator<bool>>>));
  ...
  EXPECT_FALSE((kamping::internal::is_vector_bool_v<
                std::vector<int, testing::CustomAllocator<int>>>));
  EXPECT_FALSE(kamping::internal::is_vector_bool_v<std::vector<int>&>);
  ...
  EXPECT_FALSE(kamping::internal::is_vector_bool_v<
               std::vector<kamping::kabool>&>);
  EXPECT_FALSE(kamping::internal::is_vector_bool_v<
               std::vector<kamping::kabool> const&>);
...
TEST(ContainerBasedConstBufferTest, get_basics) {
  ...
  constexpr ParameterType ptype = ParameterType::send_counts;
  ContainerBasedConstBuffer<std::vector<int>, ptype> buffer_based_on_int_vector(
    int_vec
  );
  ContainerBasedConstBuffer<std::vector<int>, ptype>
    buffer_based_on_const_int_vector(int_vec_const);
  ...
  static_assert(std::is_same_v<
                decltype(buffer_based_on_int_vector.get().data()), const int*>);
  EXPECT_EQ(buffer_based_on_int_vector.data(), int_vec.data());
  static_assert(std::is_same_v<
                decltype(buffer_based_on_int_vector.data()), const int*>);

  EXPECT_EQ(
    buffer_based_on_const_int_vector.get().size(), int_vec_const.size()
  );
  EXPECT_EQ(
    buffer_based_on_const_int_vector.get().data(), int_vec_const.data()
  );
  static_assert(std::is_same_v<
                decltype(buffer_based_on_const_int_vector.get().data()),
                const int*>);
  EXPECT_EQ(buffer_based_on_const_int_vector.data(), int_vec_const.data());
  static_assert(std::is_same_v<
                decltype(buffer_based_on_const_int_vector.data()), const int*>);
}
...
TEST(ContainerBasedConstBufferTest, get_containers_other_than_vector) {
  ...
  ContainerBasedConstBuffer<testing::OwnContainer<int>, ptype>
...
TEST(ContainerBasedConstBufferTest, move_constructor_is_enabled) {
  ...
  EXPECT_TRUE(
    std::equal(container.begin(), container.end(), buffer2.get().data())
  );
}
...
TEST(ContainerBasedOwningBufferTest, get_basics) {
  ...
  constexpr ParameterType ptype = ParameterType::send_counts;
  ContainerBasedOwningBuffer<std::vector<int>, ptype>
    buffer_based_on_moved_vector(std::move(int_vec));
  ContainerBasedOwningBuffer<std::vector<int>, ptype>
    buffer_based_on_rvalue_vector(std::vector<int>{1, 2, 3});
  ...
  static_assert(std::is_same_v<
                decltype(buffer_based_on_moved_vector.get().data()),
                int const*>);
  ...
  static_assert(std::is_same_v<
                decltype(buffer_based_on_moved_vector.data()), int const*>);
  ...
  static_assert(std::is_same_v<
                decltype(buffer_based_on_rvalue_vector.get().data()),
                const int*>);
  ...
  static_assert(std::is_same_v<
                decltype(buffer_based_on_rvalue_vector.data()), int const*>);

  {
    auto const& underlying_container =
      buffer_based_on_moved_vector.underlying();
    EXPECT_EQ(underlying_container, (std::vector<int>{1, 2, 3}));
  }
  {
    auto const& underlying_container =
      buffer_based_on_rvalue_vector.underlying();
    EXPECT_EQ(underlying_container, (std::vector<int>{1, 2, 3}));
  }
}
...
TEST(ContainerBasedOwningBufferTest, get_containers_other_than_vector) {
  ...
  ContainerBasedOwningBuffer<std::string, ptype> buffer_based_on_string(
    std::move(str)
  );
  ...
  ContainerBasedOwningBuffer<testing::OwnContainer<int>, ptype>
    buffer_based_on_own_container(std::move(own_container));
  EXPECT_EQ(own_container.copy_count(), 0);
  ...
  {
    auto const& underlying_container =
      buffer_based_on_own_container.underlying();
...
TEST(ContainerBasedOwningBufferTest, move_constructor_is_enabled) {
  ...
  EXPECT_TRUE(std::equal(
    expected_container.begin(), expected_container.end(), buffer2.get().data()
  ));
}
...
TEST(UserAllocatedContainerBasedBufferTest, resize_and_data_basics) {
  ...
  constexpr ParameterType ptype = ParameterType::send_counts;
  UserAllocatedContainerBasedBuffer<std::vector<int>, ptype>
    buffer_based_on_int_vector(int_vec);
...
TEST(
  UserAllocatedContainerBasedBufferTest,
  resize_and_data_containers_other_than_vector
) {
  testing::OwnContainer<int> own_container;

  constexpr ParameterType ptype = ParameterType::recv_counts;
  UserAllocatedContainerBasedBuffer<testing::OwnContainer<int>, ptype>
    buffer_based_on_own_container(own_container);
...
TEST(UserAllocatedContainerBasedBufferTest, move_constructor_is_enabled) {
  ...
  const auto              const_container =
    container; // ensure that container is not altered
  UserAllocatedContainerBasedBuffer<std::vector<int>, ptype> buffer1(container);
  UserAllocatedContainerBasedBuffer<std::vector<int>, ptype> buffer2(
    std::move(buffer1)
  );
  EXPECT_EQ(buffer2.get().size(), const_container.size());
  EXPECT_TRUE(std::equal(
    const_container.begin(), const_container.end(), buffer2.get().data()
  ));
}

TEST(LibAllocatedContainerBasedBufferTest, resize_and_data_extract_basics) {
  constexpr ParameterType ptype = ParameterType::recv_counts;
  LibAllocatedContainerBasedBuffer<std::vector<int>, ptype>
    buffer_based_on_int_vector;
...
TEST(
  LibAllocatedContainerBasedBufferTest, extract_containers_other_than_vector
) {
  constexpr ParameterType ptype = ParameterType::recv_counts;
  LibAllocatedContainerBasedBuffer<testing::OwnContainer<int>, ptype>
    buffer_based_on_own_container;
  ...
  const size_t last_resize = 9;
  resize_write_check(last_resize);
  testing::OwnContainer<int> underlying_container =
    buffer_based_on_own_container.extract();
  for (size_t i = 0; i < last_resize; ++i) {
    EXPECT_EQ(underlying_container[i], static_cast<int>(last_resize - i));
  }
}
...
TEST(
  LibAllocatedContainerBasedBufferTest, move_ctor_assignment_operator_is_enabled
) {
  constexpr ParameterType ptype = ParameterType::recv_counts;
  ...
  LibAllocatedContainerBasedBuffer<testing::OwnContainer<int>, ptype> buffer2(
    std::move(buffer1)
  );
...
TEST(SingleElementConstBufferTest, get_basics) {
  ...
  static_assert(std::is_same_v<
                decltype(int_buffer)::value_type, decltype(value)>);
}
...
TEST(DataBufferTest, has_extract) {
  static_assert(
    has_extract_v<DataBuffer<
      int, ParameterType::send_buf, BufferModifiability::modifiable,
      BufferOwnership::owning, BufferAllocation::lib_allocated>>,
...
TEST(LibAllocatedContainerBasedBufferTest, prevent_usage_after_extraction) {
  LibAllocatedContainerBasedBuffer<std::vector<int>, ParameterType::recv_buf>
    buffer;
  ...
  EXPECT_KASSERT_FAILS(
    buffer.extract(), "Cannot extract a buffer that has already been extracted."
  );
  EXPECT_KASSERT_FAILS(
    buffer.get(), "Cannot get a buffer that has already been extracted."
  );
  EXPECT_KASSERT_FAILS(
    buffer.data(),
  ...
  EXPECT_KASSERT_FAILS(
    buffer.resize(20), "Cannot resize a buffer that has already been extracted."
  );
}

TEST(
  LibAllocatedContainerBasedBufferTest,
  prevent_usage_after_extraction_via_mpi_result
) {
  LibAllocatedContainerBasedBuffer<std::vector<int>, ParameterType::recv_buf>
    recv_buffer;
