...
TEST(ParameterFactoriesTest, single_and_multiple_element_const_send_recv_buffer_type) {
    ...
    bool const vec_result = std::is_same_v<
        decltype(gen_int_vec_buffer),
        ContainerBasedConstBuffer<std::vector<uint8_t>, ParameterType::send_recv_buf>>;
    EXPECT_TRUE(vec_result);
}
...
TEST(ParameterFactoriesTest, make_data_buffer) {
    {
        ...
        // As this buffer is referencing, the addresses of vec ad data_buf.underlying() should be
        // the same.
    ...
    }
    {
        ...
        constexpr internal::ParameterType type = internal::ParameterType::send_buf;
        auto                              data_buf =
            internal::make_data_buffer<type, BufferModifiability::constant>(std::move(vec));
        EXPECT_EQ(data_buf.parameter_type, type);
        EXPECT_FALSE(data_buf.is_modifiable);
        EXPECT_FALSE(data_buf.is_single_element);
        static_assert(
    ...
    }
    {
        ...
        constexpr internal::ParameterType type = internal::ParameterType::send_buf;
        auto                              data_buf =
            internal::make_data_buffer<type, BufferModifiability::modifiable>(NewContainer<int>{});
        EXPECT_EQ(data_buf.parameter_type, type);
        EXPECT_TRUE(data_buf.is_modifiable);
        EXPECT_TRUE(data_buf.is_single_element);
        static_assert(
    ...
    }
    {
        ...
        constexpr internal::ParameterType type = internal::ParameterType::send_buf;
        auto                              data_buf =
            internal::make_data_buffer<type, BufferModifiability::modifiable>({1, 2, 3});
        EXPECT_EQ(data_buf.parameter_type, type);
        EXPECT_TRUE(data_buf.is_modifiable);
        EXPECT_FALSE(data_buf.is_single_element);
        static_assert(
...
TEST(ParameterFactoriesTest, make_data_buffer_boolean_value) {
    // use a custom container, because std::vector<bool> is not supported (see compilation failure
    // tests)
    {
        // Constant, container, referencing, user allocated
    ...
    }
    {
        ...
        constexpr internal::ParameterType type = internal::ParameterType::send_buf;
        auto                              data_buf =
            internal::make_data_buffer<type, BufferModifiability::constant>(single_bool);
        EXPECT_EQ(data_buf.parameter_type, type);
        EXPECT_FALSE(data_buf.is_modifiable);
        EXPECT_TRUE(data_buf.is_single_element);
        // As this buffer is referencing, the addresses of vec ad data_buf.underlying() should be
    ...
    }
    {
        ...
        constexpr internal::ParameterType type = internal::ParameterType::send_buf;
        auto                              data_buf =
            internal::make_data_buffer<type, BufferModifiability::modifiable>(NewContainer<bool>{});
        EXPECT_EQ(data_buf.parameter_type, type);
        EXPECT_TRUE(data_buf.is_modifiable);
        EXPECT_TRUE(data_buf.is_single_element);
        static_assert(
    ...
    }
    {
        ...
        constexpr internal::ParameterType type = internal::ParameterType::send_buf;
        auto                              data_buf =
            internal::make_data_buffer<type, BufferModifiability::modifiable>({true, false, true});
        EXPECT_EQ(data_buf.parameter_type, type);
        EXPECT_TRUE(data_buf.is_modifiable);
        EXPECT_FALSE(data_buf.is_single_element);
        static_assert(
    ...
    }
    {
        ...
        constexpr internal::ParameterType type = internal::ParameterType::send_buf;
        auto                              data_buf =
            internal::make_data_buffer<type, BufferModifiability::constant>({true, false, true});
        EXPECT_EQ(data_buf.parameter_type, type);
        EXPECT_FALSE(data_buf.is_modifiable);
        EXPECT_FALSE(data_buf.is_single_element);
        static_assert(
            std::
                is_same_v<decltype(data_buf)::MemberTypeWithConstAndRef, const std::vector<kabool>>,
            "Initializer lists of type bool have to be converted to std::vector<kabool>."
        );
        // extract() as proxy for lib allocated DataBuffers
        EXPECT_FALSE(has_extract_v<decltype(data_buf)>);
