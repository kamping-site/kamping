...
TEST_F(CommunicatorTest, constructor_with_mpi_communicator_and_root) {
    for (int i = -(2 * size); i < (2 * size); ++i) {
        if (i < 0 || i >= size) {
            EXPECT_THROW(
              Communicator(MPI_COMM_WORLD, i), kassert::KassertException
            );
            EXPECT_THROW(
              Communicator(MPI_COMM_NULL, i), kassert::KassertException
            );
        } else {
    ...
    for (int i = 2; i <= size; ++i) {
        ...
        for (int rank_to_test = 0; rank_to_test < size; ++rank_to_test) {
            int const expected_rank_rn_splitted_comm =
              rank_to_test % i == color ? expected_size - (rank_to_test / i) - 1
                                        : MPI_UNDEFINED;
            EXPECT_EQ(
